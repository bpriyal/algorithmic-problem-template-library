...
public:
    ...
    int minRefuelStops(int target, int startFuel, vector<vector<int>>& stations) {
        ...
        for (const auto& station : stations) {
            while (!pq.empty() && remainingFuel < (station[0] - lastStopPosition)) {
            ...
            if (remainingFuel < (station[0] - lastStopPosition)) return -1;
            remainingFuel -= (station[0] - lastStopPosition);
            pq.emplace(station[1]);
            lastStopPosition = station[0];
        }

        if (!pq.empty() && remainingFuel < (target - lastStopPosition)) {
            auto stationFuel = pq.top();
            pq.pop();
            remainingFuel += stationFuel;
            ++stops;
