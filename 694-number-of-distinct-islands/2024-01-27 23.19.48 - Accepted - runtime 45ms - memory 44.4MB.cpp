class Solution {
    ...
    void util(vector<vector<int>>& grid, int row, int col, vector<vector<int>>& coordinates_) {
        if (row < 0 || row >= _rows || col < 0 || col >= _cols) return;
        if (grid[row][col] == 0 || grid[row][col] == -1) return;
        coordinates_.emplace_back(vector<int>{row, col});
        grid[row][col] = -1;
    ...
    void printCoordinates(const std::vector<vector<int>>& coordinates) {
        ...
        for (int row = 0; row < _rows; ++row) {
            for (int col = 0; col < _cols; ++col) {
                if (grid[row][col] == 1) {
                    ...
                    subtractRootCoordinate(coordinates);
                    if (isUniqueIsland(coordinates)) _uniqueIslands.emplace_back(coordinates);
                }
            }
        }
