...
struct CustomComparator {
    // int _target;
    // CustomComparator(int target_) : _target(target_) {}
...
class Solution {
    void _closestKValues(TreeNode* root, double target, int k, PQ& pq) {
        ...
        if (pq.size() >= k) {
            // cout<<"root->val: "<<root->val<<", pq.top().first: "<<pq.top().first<<", pq.top().second: "<<pq.top().second<<endl;
...
public:
    vector<int> closestKValues(TreeNode* root, double target, int k) {
        vector<int> result;
        // CustomComparator customComparator(target);
