...
public:
    int findKthLargest(vector<int>& nums, int k) {
      ...
      while (left <= right) {
        ...
        cout << "pivotIdx: " << pivotIdx << "inflection: " << inflectionIdx << endl;
        for (int num : nums) cout << num << "\t";
        cout << endl;
        if (inflectionIdx == nums.size() - k) return nums[inflectionIdx];
        else if (inflectionIdx < nums.size() - k) left = inflectionIdx + 1;
