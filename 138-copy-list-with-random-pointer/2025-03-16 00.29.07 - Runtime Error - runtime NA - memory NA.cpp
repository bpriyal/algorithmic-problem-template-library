...
public:
    Node* copyRandomList(Node* head) {
      ...
      originalTemp = head;
      copyTemp = copyHead;
      cout << copyTemp->next->val << endl;
      while (originalTemp) {
        // if (!copyTemp) cout << "null copy temp " << endl;
        // else cout << copyTemp->val << endl;
        if (originalTemp->random) {
          auto idx = nodeToIdxMap[originalTemp->random];
          // cout << "idx: " << idx << ", val: " << copyTemp->val << endl;
